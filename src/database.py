...
class Engine(object):
    ...
    def populate_tables(self, data_path=None):
        ...
        # All farms
        farms = {entry.strip(): Farm(name=entry.strip())
                 for entry in raw_dataset["location"].unique().tolist()}

        # pH is a decimal value between 0 - 14
        ph_validated = raw_dataset[(raw_dataset.sensorType == "pH") & (
            raw_dataset.value.between(0, 14))]
