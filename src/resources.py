...
def get_farm_data_by_month(farmId: int, monthOfYear: int):
    ...
    with g.session as session:
        try:
            results = dao.FarmsDao(session).get_farm_data_by_month(
                farmId, monthOfYear)
            return create_response(200, results)
        except Exception as eror:
            return create_response(403, message="{}".format(eror))
...
def get_farm_data_by_metric(farmId, metricId):
    with g.session as session:
        try:
            results = dao.FarmsDao(
                session).get_farm_data_by_metric(farmId, metricId)
            return create_response(200, results)
        except Exception as eror:
            return create_response(403, message="{}".format(eror))


@app.route("/solitafarms/farms/<int:farmId>/metric/<int:metricId>/monthly-aggregates/", methods=["GET"])
def get_farm_data_aggregates(farmId, metricId):
    with g.session as session:
        try:
            results = dao.FarmsDao(
                session).get_farm_monthtly_metric_aggregates(farmId, metricId)
            return create_response(200, results)
        except Exception as eror:
            return create_response(403, message="{}".format(eror))
